...
namespace Wrappers
{
  ...
  struct PassInfo
  {
    std::vector<std::pair<AttachmentType, VkImageLayout>> Attachments;
  };

  class Pass
  {
    public:
      Pass(PassInfo AttachmentInfos);
  ...
  {
    public:
      Renderpass(VkDevice& Device, uint32_t PassCount, Pass* Passes, FrameBufferDescription FBDesc, VkSampleCountFlagBits);
