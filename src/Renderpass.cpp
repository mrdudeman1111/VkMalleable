...
namespace Wrappers
{
  Pass::Pass(PassInfo AttachmentInfos)
  {
    bDepthBuffer = false;
    ...
    for(uint32_t i = 0; i < AttachmentInfos.Attachments.size(); i++)
    {
      switch(AttachmentInfos.Attachments[i].first)
      {
        ...
        case AttachmentType::eInputAtt:
          ...
          break;
        case AttachmentType::ePreserveAtt:
          PreserveAttachments.push_back({});
          PreserveAttachments.front() = i;
          break;
        case AttachmentType::eResolveAtt:
